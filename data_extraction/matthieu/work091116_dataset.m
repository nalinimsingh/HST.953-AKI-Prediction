% DATASET OF PATIENTS WITH SEPSIS - MIMIC-III (v1.3)
% Code by: Matthieu Komorowski mkomo@mit.edu

% using cohort defined before (sepsis-3)
% 230816: make sure vasoMV and vasoCV are the right versions!! (no ceiling of doses!!)
% 071116: make sure Refvitals is the right version (28x10, with interface)

% STEPS:
%-------------------------------
% EXTRACT ONLY DATA for PATIENTS WITH SEPSIS-3 / right period for MDP
% PREPROCESSING
% REFORMAT in 4h time slots
% COMPUTE SOFA etc... at each time step
% CLEAN dataset for MDP
...
winb4=25;   %lower limit for inclusion of data (25h before time flag)
winaft=49;  % upper limit (49h after)
winwidth=winb4+winaft;
...
icustayid=sepsis.icustayid(ii)-200000;
qst=sepsis.sepsis_time(ii);%onset(icustayid,3); %flag for presumed infection
...
reformat2=nan(size(reformat,1),85);  %output array
...
%                    disp(icustayid)                  %moved here to save some time
    ...
    for j=0:timestep:winwidth+timestep % -25 until +49 = 74 hours in total, I add 1 timestep to not miss any data on the right hand side
