...
def calculateurineaki(urine, cohort):
    ...
    aki_urine_frame = pd.DataFrame(aki_urine, columns=['icustay_id', 'aki_urine', 'aki_urine_t'])
    aki_urine_frame['aki_urine'] = aki_urine_frame['aki_urine'].astype(bool)
    aki_urine_frame.ix[aki_urine_frame.aki_urine==False, 'aki_urine_t']=np.nan 
...
def getlrdata(cohort, eth, min_maps, creat_summary, aki_urine):
    ...
    lr_data = lr_data.join(creat_summary[['creat_aki','i_time']],how='left')
    lr_data = lr_data.join(aki_urine['aki_urine'].astype(bool), how='left')
    lr_data = lr_data.join(aki_urine['aki_urine_t'], how='left')
    lr_data.rename(columns={'value':'min_map','bin':'min_map_bin', 'i_time':'tAKI_c', 'aki_urine_t':'tAKI_u'},inplace=True)
    
    lr_data['aki'] = (lr_data['aki_urine'] | creat_summary['creat_aki'])
    
    lr_data['tAKI'] = np.nan
    for icu in lr_data.index:
        if np.isfinite(lr_data.ix[icu, 'tAKI_c']):
            if np.isfinite(lr_data.ix[icu, 'tAKI_u']):
                lr_data.ix[icu, 'tAKI'] = min(lr_data.ix[icu, 'tAKI_c'], lr_data.ix[icu, 'tAKI_u'])
            else:
                lr_data.ix[icu, 'tAKI'] = lr_data.ix[icu, 'tAKI_c']
        elif np.isfinite(lr_data.ix[icu, 'tAKI_u']):
            lr_data.ix[icu, 'tAKI'] = lr_data.ix[icu, 'tAKI_u']
